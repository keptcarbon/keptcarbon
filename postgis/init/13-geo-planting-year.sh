...
# backend/app/data/rasters/planting_year_rayong.tif, bind-mounted here
# as /rasters (see docker-compose.yml).
set -euo pipefail

RASTER=/rasters/planting_year_rayong.tif

if [ ! -f "$RASTER" ]; then
  echo "13-geo-planting-year.sh: $RASTER not found, skipping raster load" >&2
  exit 0
fi
