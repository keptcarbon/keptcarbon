-- ============================================================================
-- Migration 010 — backfill projects / plots / plot_landuse_overlaps /
--                  plot_assessments / plot_carbon_yearly from carbon_projects
-- ============================================================================
...
--   carbon_projects (header cols)                    -> projects (same id preserved)
--   frontend_plots[]                                  -> plots (one row per id -- the stable
...
--   plantation_info[], polygons_payload[]             -> plots (geometry/status/request params)
--   plantation_info[].lu_polygon[]                    -> plot_landuse_overlaps
--   backend_responses[]                               -> plot_assessments
--   backend_responses[].carbon_profile[]               -> plot_carbon_yearly
--
...
-- plots.geometry/status or plot_landuse_overlaps rows.
--
-- plots.polygon_id is deliberately sourced from frontend_plots[i].id (NOT
...
--
-- Not recoverable from the source data (left NULL): plot_assessments.
-- model_version -- carbon_projects never recorded which backend/allometry
-- version produced a result, only its output. Every plot_assessments row
...
-- migration's inserts -- the FK chain cascades from projects:
--   TRUNCATE projects, plots, plot_landuse_overlaps, plot_assessments,
--            plot_carbon_yearly RESTART IDENTITY CASCADE;
-- The guard below refuses to run over a non-empty plots table so a second
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM plots LIMIT 1) THEN
    RAISE EXCEPTION 'plots already has rows -- migration 010 already applied (or plots is not empty), aborting. See header for how to reset and re-run.';
  END IF;
END $$;

-- ============================================================================
-- 1) projects — one row per carbon_projects row, id preserved so any
...
-- ============================================================================
-- 2) plots — one row per frontend_plots[] element (the id space the live
...
-- ============================================================================
-- 3) plot_landuse_overlaps — one row per plantation_info[].lu_polygon[],
--    matched to its plot via the SAME positional correlation as step 2
--    (plantation_info[i] <-> frontend_plots[i] by array position).
-- ============================================================================
INSERT INTO plot_landuse_overlaps (plot_id, lu_class, lu_class_desc_th, geometry, area_m2, area_percent, created_at)
SELECT
  pl.id,
  lu.value->>'lu_class',
...
-- ============================================================================
-- 4) plot_assessments — one row per backend_responses[] element, matched to
--    its plot by REAL ID (backend_responses[].polygon_id == plots.polygon_id,
...
-- ============================================================================
INSERT INTO plot_assessments (plot_id, status, status_code, message, message_th, ci, assess_parameters, model_version, is_current, created_at)
SELECT
  pl.id,
...
-- ============================================================================
-- 5) plot_carbon_yearly — one row per backend_responses[].carbon_profile[],
...
BEGIN
  ...
  SELECT COUNT(*) INTO n_plots       FROM plots;
  SELECT COUNT(*) INTO n_overlaps    FROM plot_landuse_overlaps;
  SELECT COUNT(*) INTO n_assessments FROM plot_assessments;
  SELECT COUNT(*) INTO n_yearly      FROM plot_carbon_yearly;

  RAISE NOTICE 'carbon_projects: % rows -> projects: % rows', n_source_projects, n_projects;
  RAISE NOTICE 'frontend_plots[] entries: % -> plots: % rows (% skipped: missing id or geometry)',
    n_source_plots, n_plots, n_source_plots - n_plots;
  RAISE NOTICE 'plot_landuse_overlaps: % rows, plot_assessments: % rows, plot_carbon_yearly: % rows',
  ...
  LOOP
    IF mismatch.n_frontend != mismatch.n_plantation THEN
      ...
      RAISE WARNING 'carbon_projects.id=% has mismatched array lengths -- frontend_plots: %, plantation_info: % -- positional join may be wrong for this project''s geometry/status/plot_landuse_overlaps, review by hand',
  ...
  ELSE
    RAISE NOTICE 'positional join check: % of % project(s) flagged above -- review before trusting their plots.geometry/status and plot_landuse_overlaps rows', n_mismatched, n_source_projects;
  END IF;
END $$;
