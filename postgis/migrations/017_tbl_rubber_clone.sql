-- ============================================================================
-- Migration 017 — create tbl_rubber_clone
-- ============================================================================
...
--     < postgis/migrations/017_tbl_rubber_clone.sql
--
...
BEGIN
  IF EXISTS (
    ...
    WHERE table_schema = 'public' AND table_name = 'tbl_rubber_clone'
  ) THEN
    RAISE EXCEPTION 'tbl_rubber_clone already exists -- migration 017 already applied, aborting.';
  END IF;
END $$;

CREATE TABLE tbl_rubber_clone (
  ...
  clone_desc         TEXT         NOT NULL
);

INSERT INTO tbl_rubber_clone
  (clone, clone_origin, clone_use, clone_site_trait, clone_desc)
VALUES
